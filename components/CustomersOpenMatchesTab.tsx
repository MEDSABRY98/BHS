...
export default function CustomersOpenMatchesTab({ data }: CustomersOpenMatchesTabProps) {
  ...
  const exportToExcel = () => {
    ...
    csvRows.push(['Total', '', '', '', totalDebit.toFixed(2), totalCredit.toFixed(2), totalRemaining.toFixed(2), ''].join(','));
    ...
    link.setAttribute('download', `open_transactions_${new Date().toISOString().split('T')[0]}.csv`);
