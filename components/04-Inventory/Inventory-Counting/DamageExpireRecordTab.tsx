...
export default function DamageExpireRecordTab() {
    ...
    const fetchData = async (isSilent = false) => {
        ...
        try {
            ...
            console.error('Failed to load damage IC record', e);
            setError(e.message);
        } finally {
            setLoading(false);
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Damage & Expire Record');
        XLSX.writeFile(workbook, `Damage_Expire_Record_${new Date().toISOString().split('T')[0]}.xlsx`);
    };

    if (loading) return <Loading message="Loading Damage & Expire Records..." />;
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-gray-100 p-4 flex flex-wrap items-center gap-4">
                <div className="relative flex-1 min-w-[200px] group">
                    <Search className="absolute left-5 top-1/2 -translate-y-1/2 w-6 h-6 text-gray-300 group-focus-within:text-red-500 transition-colors" />
                    <input
                        type="text"
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        placeholder="Search by Product, User, Warehouse..."
                        className="w-full pl-14 pr-6 py-4 bg-slate-50/50 border border-transparent rounded-2xl text-base font-bold text-slate-700 placeholder:text-gray-300 focus:bg-white focus:border-red-500 focus:ring-4 focus:ring-red-500/10 transition-all outline-none"
                    />
                </div>
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-gray-100 overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full" style={{ tableLayout: 'fixed' }}>
                        ...
                        <thead className="bg-gradient-to-r from-red-800 to-red-950 text-white">
                            <tr>
                                    ...
                                    <tr key={idx} className="hover:bg-red-50/30 transition-all group border-b border-gray-50">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <div className="w-6 h-6 rounded-full bg-red-50 flex items-center justify-center text-[10px] font-black text-red-500">
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className="inline-flex px-2 py-1 rounded-md bg-rose-50 text-rose-600 text-[10px] font-black uppercase">
                                                {item.warehouse}
                                            </span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
                                            <span className="text-xs font-black text-slate-700 truncate block">{item.barcodeName}</span>
                                        </td>
                                        <td className="px-4 py-4 text-center">
