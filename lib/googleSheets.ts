  ...
  try {
    ...
    return rows.slice(1)
      ...
      .map((row, i) => ({
        ...
        lpoDeliveryDate: row[3]?.toString() || '',  // D: LPO Delivery Date (NEW)
        customerName: row[4]?.toString() || '',     // E: Customer
        lpoValue: parseFloat(row[5]?.toString().replace(/,/g, '') || '0'), // F
        invoiceDate: row[6]?.toString() || '',      // G
        invoiceNumber: row[7]?.toString() || '',    // H
        invoiceValue: parseFloat(row[8]?.toString().replace(/,/g, '') || '0'), // I
        status: (row[9]?.toString() || 'pending') as LpoRecord['status'],  // J
        reship: ['true', 'yes', 'TRUE', 'YES'].includes(row[10]?.toString()?.trim() || ''), // K
        notes: row[11]?.toString() || '',           // L
        createdAt: row[12]?.toString() || '',       // M
        updatedAt: row[13]?.toString() || '',       // N
      }));
  } catch (error) {
...
export async function updateCustomerDocument(rowIndex: number, data: Partial<any>) {
  try {
    const credentials = getServiceAccountCredentials();
    const auth = new google.auth.GoogleAuth({
      credentials,
    ...
    // Map fields to columns (A is 0, B is 1, etc.)
    // A: CUSTOMER NAME, B: CREDIT APP, C: LICENCE, D: LICENCE DATE, E: TRN, F: PASSPORT, G: PASSPORT DATE, H: ID, I: ID DATE
    const updates = [];
    if (data.creditApp !== undefined) updates.push({ range: `'Customers Documents'!B${rowIndex}`, values: [[data.creditApp]] });
    if (data.licence !== undefined) updates.push({ range: `'Customers Documents'!C${rowIndex}`, values: [[data.licence]] });
    if (data.licenceDate !== undefined) updates.push({ range: `'Customers Documents'!D${rowIndex}`, values: [[data.licenceDate]] });
    if (data.trn !== undefined) updates.push({ range: `'Customers Documents'!E${rowIndex}`, values: [[data.trn]] });
    if (data.passport !== undefined) updates.push({ range: `'Customers Documents'!F${rowIndex}`, values: [[data.passport]] });
    if (data.passportDate !== undefined) updates.push({ range: `'Customers Documents'!G${rowIndex}`, values: [[data.passportDate]] });
    if (data.id !== undefined) updates.push({ range: `'Customers Documents'!H${rowIndex}`, values: [[data.id]] });
    if (data.idDate !== undefined) updates.push({ range: `'Customers Documents'!I${rowIndex}`, values: [[data.idDate]] });
