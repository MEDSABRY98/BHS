...
export async function getMixDebit() {
  ...
  try {
    customersData = await fetchAllData(() => bhs_supabase.from('bhs_CUSTOMERS').select('"CUSTOMER ID", "CUSTOMER MAIN NAME", "CUSTOMER CITY", "CREDIT LIMIT", "CUSTOMER TAG", "CUSTOMER CLASS"'));
  } catch (err) {
    customersError = err;
  }
