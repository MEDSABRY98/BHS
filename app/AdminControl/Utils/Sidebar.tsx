...
const TABS = [
  ...
  { id: 'by-module', label: 'By Module', icon: Boxes },
    ...
    <div className="flex flex-col h-full bg-[#0a0f1d] text-white border-r border-indigo-950/20">
      ...
      <div className={`px-4 ${isCollapsed ? 'py-4' : 'pt-2 pb-6'} shrink-0 flex flex-col items-center justify-center transition-all duration-300 border-b border-white/5`}>
        <div className="flex flex-col items-center text-center">
          <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-indigo-600 rounded-2xl flex items-center justify-center mb-3 shadow-lg shadow-indigo-950/50 transition-all duration-300">
            <LayoutGrid className="w-6 h-6 text-white" />
          </div>
          {!isCollapsed && (
            <div className="animate-in fade-in duration-300">
              <h2 className="text-lg font-bold tracking-tight text-white">Admin Control</h2>
              <p className="text-[10px] text-violet-400 font-bold tracking-[0.2em] uppercase">BHS Panel</p>
            </div>
          )}
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              {!isCollapsed && isActive && (
                <ChevronRight className="w-4 h-4 ml-auto text-violet-300 animate-in fade-in duration-200" />
              )}
            </button>
          );
        })}
