...
export default function InventoryProductsBalanceTab() {
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent('inventory-analysis-refresh-state', {
        detail: { activeTab: 'products_balance', isRefreshing: loading },
      })
    );
  }, [loading]);
  ...
  useEffect(() => {
    const handleTrigger = (e: Event) => {
      ...
      if (customEvent.detail?.activeTab === 'products_balance') {
    ...
    try {
      ...
      if (!opts?.skipCache && isDefaultBalanceFilters(from, to, location)) {
        ...
        if (prefetched?.productsBalance) {
          ...
          setData(prefetched.productsBalance);
          setError(null);
          setLoading(false);
          return;
        }
  ...
  const activeFiltersCount = useMemo(() => {
    ...
    if (selectedBalanceFilter !== 'All') count++;
    if (appliedDateFrom !== '' || appliedDateTo !== '') count++;
    return count;
  }, [selectedLocation, selectedCategory, selectedBalanceFilter, appliedDateFrom, appliedDateTo]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* KPI Metrics Cards */}
      <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 transition-opacity duration-300 ${loading && data.length > 0 ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          >
            ...
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-black text-slate-800">Filter Products Balance</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsFiltersOpen(false)}
            ...
            <div className="space-y-4">
              ...
              {/* Ending Balance Filter */}
              <div className="space-y-1.5" ref={balanceFilterRef}>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-wider">
                  Ending Balance
                </label>
                <div className="relative">
                  <button
                    ...
                    className={`w-full flex items-center justify-between h-11 bg-slate-50 hover:bg-slate-100/60 border-2 border-transparent rounded-2xl px-4 transition-all shadow-sm text-sm font-bold text-slate-800 cursor-pointer ${
                      isBalanceFilterOpen ? 'bg-white border-indigo-500 ring-2 ring-indigo-500/10' : ''
                    }`}
                  >
                    <div className="flex items-center gap-2 truncate">
                      <Layers className="w-4 h-4 text-slate-400 shrink-0" />
                      <span className="text-xs font-bold text-slate-800 truncate">
                        {selectedBalanceFilter === 'All' ? 'All Balances' : BALANCE_FILTER_OPTIONS.find(o => o.value === selectedBalanceFilter)?.label}
                      </span>
                    </div>
                  ...
                  {isBalanceFilterOpen && (
                    <div className="absolute left-0 right-0 top-12 z-50 bg-white border border-slate-200 shadow-2xl rounded-2xl p-2 animate-in fade-in duration-100">
