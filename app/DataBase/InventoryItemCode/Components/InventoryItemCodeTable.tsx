...
import { Hash, Edit2, Trash2 } from 'lucide-react';
import NoData from '@/app/Components/NoDataTab';

export interface InventoryItemCodeRow {
  ID: string;
  TAGS: string | null;
...
interface Props {
  rows: InventoryItemCodeRow[];
  isLoading: boolean;
  canEdit: boolean;
  canDelete: boolean;
  onEdit: (row: InventoryItemCodeRow) => void;
  onDelete: (id: string) => void;
}

export default function InventoryItemCodeTable({
  rows,
  isLoading,
  canEdit,
  canDelete,
  ...
  if (rows.length === 0) {
    return <NoData title="NO ITEM CODES FOUND" />;
  }
        ...
        >
          <div className="flex justify-between items-start gap-3">
            <div className="min-w-0 flex-1">
              <span className="text-xs font-bold text-gray-400 tracking-wider font-mono">{row.ID}</span>
              <h3 className="text-lg font-black text-black mt-1 leading-tight truncate font-mono" title={row['ITEM CODE'] || ''}>
                {row['ITEM CODE'] || '—'}
              </h3>
              {row.BARCODE && (
                <p className="text-[10px] font-bold text-gray-400 mt-1 truncate font-mono">{row.BARCODE}</p>
              )}
            </div>
            <div className="bg-gray-50 border border-gray-100/50 px-3 py-1.5 rounded-2xl text-center shrink-0 max-w-[90px]">
              <span className="text-[10px] font-black text-black truncate block" title={row.TAGS || ''}>
                {(row.TAGS || '—').split(',')[0]}
              </span>
              <p className="text-[8px] font-black text-gray-400 uppercase tracking-wider mt-0.5">Tag</p>
            </div>
          </div>
