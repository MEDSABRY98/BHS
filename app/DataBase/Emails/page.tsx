...
export default function EmailsDatabasePage() {
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      ...
      await fetch(`/DataBase/Emails/api?\${queryParams.toString()}`, { method: 'DELETE' });
      fetchData();
    } catch (error) {
      console.error(error);
    }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 text-xs uppercase tracking-wider text-gray-500">
                ...
                <th className="px-6 py-4 font-medium border-b border-gray-100">Email Name</th>
              ...
              {loading ? (
                ...
                filteredData.map((item, idx) => (
                  <tr key={item.ID || idx} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{item['EMAIL_NAME']}</td>
