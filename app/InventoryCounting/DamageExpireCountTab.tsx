...
export default function DamageExpireCountTab() {
    ...
    const fetchData = async (isSilent = false) => {
        ...
        setError(null);
        try {
            const result = await fetchICCountTabData('DamageExpire');
                        ...
                        <tbody className="divide-y divide-gray-100">
                                {filteredData.map((item, idx) => (
                                    <tr 
                                        ...
                                        className="hover:bg-red-50/50 transition-all group cursor-pointer"
                                    >
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className={`inline-flex px-3 py-1.5 rounded-xl text-sm font-black border shadow-sm ${item.countedQty > 0 ? 'bg-red-50 text-red-700 border-red-100' : 'bg-slate-50 text-slate-400 border-slate-100'
                                                }`}>
