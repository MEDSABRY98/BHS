...
export default function NormalCountTab() {
    ...
    const fetchData = async (isSilent = false) => {
        ...
        try {
            const [totalRes, recordRes] = await Promise.all([
                fetch('/api/Inventory/Counting/NormalTotal'),
                fetch('/api/Inventory/Counting/NormalRecord')
            ]);
    ...
    const handleSaveItem = async (updatedValues: Partial<ICItem>) => {
        ...
        setData(prev => prev.map(p => 
            p.productId === editingItem.productId 
                ? { ...p, ...updatedValues }
                : p
        ));
    };
                    ...
                    <table className="w-full" style={{ tableLayout: 'fixed' }}>
                        ...
                        <tbody className="divide-y divide-gray-100">
                                {filteredData.map((item, idx) => (
                                    ...
                                    >
                                        <td className="px-4 py-4 text-center text-sm font-bold text-slate-400 group-hover:text-blue-600 transition-colors">{idx + 1}</td>
                                        <td className="px-4 py-4 text-center text-sm font-black text-slate-600 transition-colors group-hover:text-blue-700 truncate">{item.barcodeName}</td>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            <span className={`inline-flex px-3 py-1.5 rounded-xl text-sm font-black border shadow-sm ${item.countedQty > 0 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-slate-50 text-slate-400 border-slate-100'
                                                }`}>
                                        ...
                                        <td className="px-4 py-4 text-center">
                                            {item.countedQty > 0 ? (
                                                <span className={`text-sm font-black ${item.countedQty - item.availableQty === 0
                                                    ? 'text-emerald-600'
