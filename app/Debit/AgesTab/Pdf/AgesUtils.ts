...
export async function generateSingleRegionAgesPDF(
  regionName: string,
  regionData: Array<{
    customerName: string;
    cities: string[];
  ...
  doc.text('Al Marai Al Arabia Trading Sole Proprietorship L.L.C', pageWidth / 2, yPosition, { align: 'center' });
  doc.setTextColor(0, 0, 0); yPosition += 7;
  doc.setFontSize(18); doc.text(`Aging Report | ${regionName}`, pageWidth / 2, yPosition, { align: 'center' });
  yPosition += 10;
  doc.setFontSize(10); doc.setFont('helvetica', 'normal');
  const now = new Date();
  const currentDate = `${now.getDate().toString().padStart(2, '0')}/${(now.getMonth() + 1).toString().padStart(2, '0')}/${now.getFullYear()}`;
...
export async function generateCombinedTagsAgesPDF(
  filteredData: Array<{
    customerName: string;
    cities: string[];
    customerTags: string[];
  ...
  filterDescription: string = 'All Tagged Customers'
) {
  const jsPDFModule = await import('jspdf');
  const jsPDF = jsPDFModule.default;
  const autoTableModule = await import('jspdf-autotable');
  ...
  const tagGroups = new Map<string, typeof filteredData>();
  filteredData.forEach(item => {
    if (item.customerTags && item.customerTags.length > 0) {
  ...
  for (const tag of sortedTags) {
    ...
    doc.text('Al Marai Al Arabia Trading Sole Proprietorship L.L.C', pageWidth / 2, yPosition, { align: 'center' });
    doc.setTextColor(0, 0, 0); yPosition += 7;
    doc.setFontSize(18); doc.text(`Aging Report | Tag: ${tag}`, pageWidth / 2, yPosition, { align: 'center' });
    yPosition += 10;
    doc.setFontSize(10); doc.setFont('helvetica', 'normal');
    const now = new Date();
    const currentDate = `${now.getDate().toString().padStart(2, '0')}/${(now.getMonth() + 1).toString().padStart(2, '0')}/${now.getFullYear()}`;
