...
interface DeletePaymentReconciliationSessionModalProps {
...
export default function DeletePaymentReconciliationSessionModal({
  session,
  isDeleting = false,
  onClose,
  onConfirm,
}: DeletePaymentReconciliationSessionModalProps) {
  return (
    <div className="fixed inset-0 z-[10050] flex items-center justify-center p-4 animate-in fade-in duration-200">
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="delete-payment-session-title"
      >
        <button
          type="button"
          onClick={onClose}
        ...
        <h3 id="delete-payment-session-title" className="text-xl font-black text-slate-900 pr-8">
          Delete saved session?
        </h3>
        <p className="text-sm text-slate-500 font-medium mt-2 leading-relaxed">
          Delete saved session{' '}
          <strong className="text-slate-800">{session.sessionId}</strong>? This will permanently
          remove {session.lineCount.toLocaleString()} invoice line
          {session.lineCount === 1 ? '' : 's'}.
        </p>
        ...
        <div className="flex gap-3 mt-8">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 py-3 bg-rose-600 text-white hover:bg-rose-700 rounded-2xl font-black text-xs uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isDeleting ? (
              <>
