...
import {
  savePaymentReconciliationSession,
  type PaymentReconciliationSaveHeader,
  type PaymentReconciliationSaveLine,
} from '../Service/debit_service';

interface SavePaymentReconciliationModalProps {
...
export default function SavePaymentReconciliationModal({
  header,
  lines,
  sessionId,
  onClose,
  onSuccess,
}: SavePaymentReconciliationModalProps) {
  const isUpdate = Boolean(sessionId?.trim());
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async () => {
    setIsSubmitting(true);
    try {
      const result = await savePaymentReconciliationSession({
        sessionId: isUpdate ? sessionId!.trim() : undefined,
        header,
        lines,
      });
      ...
      if (result.updated) {
        toast.success(`Updated ${result.sessionId} (${result.rowCount} invoice line(s))`);
      } else {
        toast.success(`Saved as ${result.sessionId} (${result.rowCount} invoice line(s))`);
      }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-black text-slate-900">
                {isUpdate ? 'Update Payment Session' : 'Save Payment Session'}
              </h3>
