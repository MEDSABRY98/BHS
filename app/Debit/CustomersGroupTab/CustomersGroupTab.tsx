...
export default function CustomersGroupTab({ data }: CustomersGroupTabProps) {
  ...
  const handlePDFExport = async (isPrint = false) => {
    ...
    try {
      const { generateGroupStatementPDF } = await import('./Pdf/GroupStatementUtils');
      await generateGroupStatementPDF(statementTitle, groupCustomers, invoicesForSummary, isPrint);
    } catch (error) {
                    ...
                    {filteredDropdownCustomers.length === 0 ? (
                      ...
                      filteredDropdownCustomers.map((cust) => {
                        ...
                        return (
                          <label
                            key={cust}
                            className={`flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 transition-colors cursor-pointer text-sm font-medium ${isSelected ? 'bg-blue-50/30 text-blue-700' : 'text-slate-600'
                              }`}
                          >
                            <input
                              type="checkbox"
