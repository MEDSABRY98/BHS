...
export default function OpenTransactionsTab({ data }: CustomersOpenMatchesTabProps) {
  ...
  const exportToExcel = async () => {
    ...
    await exportDebitExcelTable(
      headers,
      rows,
      `open_transactions_${new Date().toISOString().split('T')[0]}`,
      {
        sheetName: 'Open Items',
        numericColumns: ['Debit', 'Credit', 'Remaining Amount'],
      }
    );
  };
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            <span className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs font-black border border-blue-100 shadow-sm">
              {filteredItems.length.toLocaleString()} {debouncedSearch && `of ${openMatches.length.toLocaleString()}`} Items
            </span>
          </div>
        </div>
                      ...
                      <tr key={headerGroup.id} className="text-center">
                        {headerGroup.headers.map((header) => {
                          const getWidth = () => {
                            ...
                            if (columnId === 'remainingAmount') return '13%';
