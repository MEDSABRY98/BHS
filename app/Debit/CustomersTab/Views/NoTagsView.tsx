...
const NoTagsView: React.FC<NoTagsViewProps> = ({
  table,
  ...
  const untaggedData = useMemo(() => {
    return filteredData.filter(c => !c.customerTags || c.customerTags.size === 0);
  }, [filteredData]);

  const untaggedRows = useMemo(() => {
    return table.getRowModel().rows.filter(row => !row.original.customerTags || row.original.customerTags.size === 0);
        ...
        <div className="bg-white rounded-xl border-2 border-gray-200 shadow-[0_4px_12px_rgba(0,0,0,0.08)] mt-6 overflow-hidden">
          <div className="py-3 px-4">
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4 items-center">
              ...
              <div className="md:col-span-1">
                <p className={`text-xl font-bold text-center ${untaggedData.reduce((sum, c) => sum + c.netDebt, 0) > 0 ? 'text-red-600' : untaggedData.reduce((sum, c) => sum + c.netDebt, 0) < 0 ? 'text-green-600' : 'text-gray-600'}`}>
                  {untaggedData.reduce((sum, c) => sum + c.netDebt, 0).toLocaleString('en-US')}
                </p>
              </div>
              <div className="md:col-span-1 flex items-center justify-center">
                <div className="text-center">
                  <span className="text-lg font-bold text-gray-700">
                    {(() => {
                      const c = untaggedData.filter(d => d.avgPaymentInterval);
                      if (!c.length) return '-';
              ...
              <div className="md:col-span-1"><p className="text-xl font-bold text-blue-600 text-center" title="Total Customers Count">{untaggedData.length}</p></div>
            </div>
          </div>
        </div>
        </>
