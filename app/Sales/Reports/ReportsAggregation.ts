...
export function applyGeoFilters(data: SalesItemWithDate[], filters?: SalesFilters): SalesItemWithDate[] {
  ...
  if (filters.customerTag) result = result.filter((i) => i.customerTag === filters.customerTag);
  if (filters.customerClass) result = result.filter((i) => i.customerClass === filters.customerClass);
...
/**
 * Builds a period for a given year/month. When `dayRange` is provided the period
 * is clamped to the same day-of-month window (e.g. 5→12) so comparison periods
 * mirror a partial current selection. Days beyond the target month's length are
 * clamped to its last day. When `dayRange` is omitted the full month is used.
 */
function buildMonthPeriod(year: number, month: number, dayRange?: DayRange | null) {
  const lastDay = new Date(year, month, 0).getDate();
  const fromDay = dayRange ? Math.min(Math.max(dayRange.fromDay, 1), lastDay) : 1;
  const toDay = dayRange ? Math.min(Math.max(dayRange.toDay, fromDay), lastDay) : lastDay;
  const from = new Date(year, month - 1, fromDay);
  const to = new Date(year, month - 1, toDay, 23, 59, 59, 999);
  const label = dayRange
    ? `${MONTH_NAMES_LONG[month - 1]} ${fromDay}–${toDay}, ${year}`
...
export function resolveReportPeriod(filters?: SalesFilters): {
  ...
  if (filters?.dateFrom || filters?.dateTo) {
    ...
    // Only derive a day-range window when the selection is within a single month
    // and is NOT already a full month; otherwise compare against full months.
    let dayRange: DayRange | null = null;
    const sameMonth =
    ...
    if (sameMonth) {
      ...
      if (!isFullMonth) {
        dayRange = { fromDay: from.getDate(), toDay: to.getDate() };
      }
    }

    const label = dayRange
      ? `${MONTH_NAMES_LONG[from.getMonth()]} ${dayRange.fromDay}–${dayRange.toDay}, ${from.getFullYear()}`
      : `${MONTH_NAMES_LONG[from.getMonth()]} ${from.getFullYear()} – ${MONTH_NAMES_LONG[to.getMonth()]} ${to.getFullYear()}`;
...
export function getPrevMonthPeriod(year: number, month: number, dayRange?: DayRange | null) {
  let py = year;
  let pm = month - 1;
  if (pm < 1) {
    pm = 12;
  ...
  return buildMonthPeriod(py, pm, dayRange);
}

export function getSameMonthLastYearPeriod(year: number, month: number, dayRange?: DayRange | null) {
