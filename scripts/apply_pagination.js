...
if (!code.includes('export async function fetchAllData')) {
  code = code.replace(/export const bhs_supabase = bhs_supabas;/, 'export const bhs_supabase = bhs_supabas;\n' + fetchHelper);
}

// 1. Update getMixDebit
code = code.replace(
  /const \{ data: debitData, error: debitError \} = await bhs_supabase\.from\('mix_DEBIT'\)\.select\('\*'\);/,
  `let debitData: any[] = [];
  let debitError = null;
  try {
...
// 2. Update bhs_CUSTOMERS in getMixDebit
code = code.replace(
  /const \{ data: customersData, error: customersError \} = await bhs_supabase\s*\.from\('bhs_CUSTOMERS'\)\s*\.select\('"CUSTOMER ID", "CUSTOMER MAIN NAME"'\);/,
